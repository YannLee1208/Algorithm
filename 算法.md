...
* Bubble
  * 从头往后相邻比较，越大的越往后移动。每次把**当前最大的移动到最后。**
  * 提前终止
    * 设置一个flage，当遍历一次发现没有交换时就终止
* Insert
  * 从后往前相邻比较，**类比于抽牌，每次抽到的牌放到合适的位置**
  * **没有提前终止，因为每次都得和前一个比较一次，实际上最优就是每个都和自己前面的比较了一次**
* 区别
  1. Bubble能够每次确定找到一个最大的放到固定位置，而Insert只是找到每个元素当前的合适位置
  2. Bubble的提前终止需要设立一个独立变量来判断，而Insert没有提前终止机制
  3. Bubble是从前往后，初始遍历n次后面逐渐变小。Insert是从后往前，初始遍历前两个逐渐变大。
...
* 思路
  ...
  * 自顶向下
    ...
    1. 自顶向上下分开，直到分到单个元素为止

    2. 自底向上递归merge **两个数组都是排好序的**

       **1. 复制一个和两个数组一样大的数组** ： 缺陷, O(n) 空间复杂度

       2. 设置三个索引，一个索引遍历原数组，另两个索引从头遍历分别在复制的两个part，挑出两个里面小的元素。

  * 自底向上（非递归）

    * 每次设立的merge块的长度 从 1 -> len / 2
    ...
    * ```c++
      ...
      *  `i + sz < n`  保证了每次merge有两部分

      * `min(i + 2*sz - 1, n - 1)`  保证了右边不越界
...
* 优化
  ...
  * 元素量很小的时候用InsertionSort
    * 数据量很小时，近乎有序的可能性更大
    * 插入排序的时间复杂度的常系数小于归并排序

* 性质
  * 稳定性
    * 稳定的，因为只要设置merge时，相等的话把左边部分的放进去就行
  * **不需要交换数组元素**
    * 链表的排序
  * **层数与原本的order无关，和quicksort不同**
...
* 两路快排
  ...
  * 为什么要一个从头一个从尾？
    ...
    *  如果 i 和 j都是从头开始的话
      ...
      * 那么实际上 **arr[l + 1, .. j] < v, arr[j+1, .. r] >= v**
        * 改为 `arr[i] <= v` 仍然有一边是等于
      * **当遇到有大量重复元素的时候，就会出现不平衡**
  ...
  * **搞清楚自己设立的index**
    ...
    * ```c++
      int _partition(Elem arr[], int l, int r){
        swap( arr[l] , arr[rand()%(r-l+1) + l]);
        Item v = arr[l];
...
* 性质
  ...
  * 稳定性
    ...
    * 最原始的快排
      ...
      * 5 | 3 1 2 | 9 7 8 9 | 4 6 3  -> 4和9交换 这样原本在左边的9就到了右边

    * 优化后的

      * 每次随机选pivot的时候，和l交换就打破了原本的顺序
...
* 统一的非递归理解
  * if_print 理解为 
    * 如果是false 就是 go 命令
    * true 就是 print命令 即 push_back
  * **if_print == false 的情况就理解为 xxxorder(node), true的情况理解为 visit(order)**
* 只需要交换 push左右和 push true的顺序即可
...
1. 构建原理
   1. 频率相关
      * 摩斯密码中，出现频率最高的e对应的密码是最简单的
   2. 前缀不重复 （prefix-free）
      * 编码中，**每个字符的编码都不是其他字符编码的前缀(prefix-free)**
      * **等价于 翻译方式唯一**
        * 假设 not prefix-free， 读到了 A， 继续读下去可以读到B则翻译不唯一
        * 假设翻译唯一，若not prefix-free，那么A以后还可以读到B拿来翻译
      * **前缀不重复在二进制编码就对应了二叉树**
        * **每个字符都是一个叶子节点**
        * **是一个full binary tree（某院独特的定义）， 即非叶子节点都有左右孩子**
          * 该性质实际上不是前缀不重复所必须的
2. Huffman code
   * 结合**频率相关**和**前缀不重复（prefix-free）**，构建一颗针对某一文本最优的二叉树，我们把这棵树就叫哈夫曼树（Huffman tree）
   * 步骤
     * 统计文本中字符出现的次数
     * 将字符按照频数升序排序
